                ...
                <div class="bg-white overflow-hidden shadow rounded-lg">
                    <div class="px-4 py-5 sm:p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    <dt class="text-sm font-medium text-gray-500 truncate">Trip Mendatang</dt>
                                    <dd class="text-2xl font-semibold text-gray-900">{{ $upcomingTrips->count() }}</dd>
                                </dl>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="text-2xl font-semibold text-gray-900">{{ $userBookings->where('status', '!=', 'cancelled')->count() }}</dd>
                                </dl>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    @if($userBookings->count() > 0)
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($userBookings as $booking)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ \Carbon\Carbon::parse($booking->trip->departure_date)->format('d M Y') }}</td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <a href="{{ route('user.bookings.show', $booking->id) }}" class="text-indigo-600 hover:text-indigo-900">Lihat</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="mt-4">
                            <a href="{{ route('user.bookings.index') }}" class="text-blue-600 hover:text-blue-900">Lihat semua booking</a>
                        </div>
                    @else
                        <p class="text-gray-500">Anda belum memiliki booking.</p>
                        <a href="{{ route('search.form') }}" class="mt-2 inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Cari dan Pesan Trip</a>
                    @endif
                </div>
            </div>
        </div>
