            ...
            <div class="bg-white border border-slate-200 rounded-2xl shadow-sm">
                <div class="p-6 text-gray-900">
                    <div class="mb-6 border-b border-slate-100 pb-4">
                        <h1 class="text-2xl font-bold text-slate-900">Tambah Trip Baru</h1>
                        <p class="text-sm text-slate-500">Tentukan kombinasi rute, bus, dan jadwal keberangkatan.</p>
                    </div>
                    ...
                    <form action="{{ route('admin.trips.store') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="route_id" id="route_id" class="mt-2 block w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/40 @error('route_id') border-rose-400 @enderror" required>
                                ...
                                @foreach($routes as $route)
                                    <option value="{{ $route->id }}" @selected(old('route_id') == $route->id)>{{ $route->origin_city }} - {{ $route->destination_city }}</option>
                                @endforeach
                            </select>
                            @error('route_id')
                        ...
                        <div>
                            ...
                            <select name="bus_id" id="bus_id" class="mt-2 block w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/40 @error('bus_id') border-rose-400 @enderror" required>
                                ...
                                @foreach($buses as $bus)
                                    <option value="{{ $bus->id }}" @selected(old('bus_id') == $bus->id)>{{ $bus->name }} ({{ $bus->plate_number }})</option>
                                @endforeach
                            </select>
                            @error('bus_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input type="date" name="departure_date" id="departure_date" value="{{ old('departure_date') }}" class="mt-2 block w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/40 @error('departure_date') border-rose-400 @enderror" required min="{{ date('Y-m-d') }}">
                        ...
                        <div>
                            ...
                            <input type="time" name="departure_time" id="departure_time" value="{{ old('departure_time') }}" class="mt-2 block w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/40 @error('departure_time') border-rose-400 @enderror" required>
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                ...
                                <input type="number" step="1000" name="price" id="price" value="{{ old('price') }}" class="mt-2 block w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/40 @error('price') border-rose-400 @enderror" required min="0">
                                @error('price')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="total_seats" id="total_seats" value="{{ old('total_seats') }}" class="mt-2 block w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/40 @error('total_seats') border-rose-400 @enderror" required min="1">
                                @error('total_seats')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
